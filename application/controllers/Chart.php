...
class Chart extends CI_Controller
{
    public function index()
    {
        ...
        $output['keuntungan_sales'] = $this->keuntungan_sales($data);
        $output['keuntungan_produk'] = $this->keuntungan_produk($data);
    ...
    function keuntungan_sales($data)
    {
        $result = array();
        foreach ($data as $row) {
            if (isset($result[$row->Rep]) == false) {
                $result[$row->Rep] = (float)$row->Total;
            } else {
                $total = $result[$row->Rep];
                $result[$row->Rep] = (float)$total + (float)$row->Total;
            }
        };
        //sorting data berdasarkan value array secara menurun
        arsort($result);
        ...
        $tabs = [['Sales', 'Total']];
        foreach ($keys as $row) {
            $dt = [$row, (float)$result[$row]];
            array_push($tabs, $dt);
        }
        return json_encode($tabs);
    }
    function keuntungan_produk($data)
    {
        $result = array();
        foreach ($data as $row) {
            if (isset($result[$row->Item]) == false) {
                $result[$row->Item] = (float)$row->Total;
            } else {
                $total = $result[$row->Item];
                $result[$row->Item] = (float)$total + (float)$row->Total;
            }
        };
        //sorting data berdasarkan value array secara menurun
        arsort($result);
